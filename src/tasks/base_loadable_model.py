...
class BaseLoadableModel(object):
    def __init__(self, config):
        super(BaseLoadableModel, self).__init()
        self.work_dir = config["work_dir"]
        self.ckpt_base_dir = config.get("ckpt_base_dir", os.path.join(self.work_dir, "ckpts"))
        
    
    def load_ckpt(self, ckpt_base_dir, current_model_name=None, model_name='model', force=True, strict=True):
        if current_model_name is None:
            current_model_name = model_name
        load_ckpt(self.__getattr__(current_model_name), ckpt_base_dir, current_model_name, force, strict)
